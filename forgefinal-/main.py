...
@app.get("/Video_Project.mp4")
def serve_video():
    return FileResponse("Video_Project.mp4", media_type="video/mp4")
...
@app.post("/api/select-brand-name")
def select_brand_name(
    request: SelectNameRequest,
    authorization: str = Header(None)
):
    session_id = get_session_from_auth_dependency(authorization)
    if not session_id:
    ...
    session["brand_name"] = request.selected_name
    save_sessions()
    return {"success": True, "brand_name": request.selected_name}

@app.post("/api/generate-logo-image")
def generate_specific_logo_image(
    request: LogoImageRequest,
    authorization: str = Header(None)
):
    """Generate image for the exact chosen logo style — not a random one"""
    session_id = get_session_from_auth_dependency(authorization)
    if not session_id:
        return {"error": "No active brand session"}
    session = brand_sessions.get(session_id)
    ...
    try:
        image_base64 = generate_logo_image(request.prompt)
        if isinstance(image_base64, str) and image_base64.startswith("Error"):
            return {"error": image_base64}
        session["logo_prompt"] = request.prompt
        ...
        print(f"Logo image generation failed: {e}")
